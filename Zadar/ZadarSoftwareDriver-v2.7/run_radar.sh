...
if [ "$radar_type" == "zprime-edge" ] || [ "$radar_type" == "zprime" ]; then
    ...
    sleep 2
    # Run cmd
    command="lib/Zadar_Run-$arch_type.AppImage --radar_num $radar_num --radar_sn $radar_sn --json $json_param --bin $bin_param --enable_viewer $enable_viewer --enable_log $enable_log"
...
if [ "$record_always" == "true" ]; then
    ...
    command_record="lib/Zadar_Record_csv-$arch_type.AppImage --radar_number $radar_num --scan off --odometry off --clusters off --tracks on &"
    echo "$command_record"
    eval "$command_record"
    record_process_pid=$!
fi
